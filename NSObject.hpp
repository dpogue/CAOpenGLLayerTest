...
protected:
    ...
    template <typename _Ret, typename... _Args>
    static _Ret sendMessageSuper(const void* pObj, SEL selector, _Args... args);
...
_NS_INLINE _Ret NS::Object::sendMessage(const void* pObj, SEL selector, _Args... args)
{
    ...
    const SendMessageProc pProc = reinterpret_cast<SendMessageProc>(objc_msg_lookup((id)pObj, selector));
...
template <typename _Ret, typename... _Args>
_NS_INLINE _Ret NS::Object::sendMessageSuper(const void* pObj, SEL selector, _Args... args)
{
    struct objc_super super = {
        (id)pObj,
        class_getSuperclass(object_getClass((id)pObj))
    };

#if defined(__GNU_LIBOBJC__)
    using SendMessageProc = _Ret (*)(const struct objc_super*, SEL, _Args...);

    const SendMessageProc pProc = reinterpret_cast<SendMessageProc>(objc_msg_lookup_super(&super, selector));
    ...
    if constexpr (doesRequireMsgSendStret<_Ret>())
    {
        using SendMessageProcStret = void (*)(_Ret*, const struct objc_super*, SEL, _Args...);

        const SendMessageProcStret pProc = reinterpret_cast<SendMessageProcStret>(&objc_msgSendSuper_stret);
        _Ret                       ret;

        (*pProc)(&ret, &super, selector, args...);
...
#endif // !defined( __arm64__ )
    {
        ...
        const SendMessageProc pProc = reinterpret_cast<SendMessageProc>(&objc_msgSendSuper);
